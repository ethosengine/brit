...
use brit_epr::elohim::{
    attestation::reach::{compute_reach, ReachInput},
    refs::BritRefManager,
};
...
pub fn compute(repo: &Path, step: &str) -> anyhow::Result<()> {
    ...
    let validation_attestations = refs
        .list_validate_refs(None)?
        .into_iter()
        .filter(|name| name.starts_with(&format!("{step}/")))
        .map(|name| name.trim_start_matches(&format!("{step}/")).to_string())
        .collect::<Vec<_>>();

...
| reset | — | absent | — | — |
| commit | — | partial | [commit.sh](../../tests/journey/parity/commit.sh) | TODO scaffold for the porcelain `git commit` flag surface (manpage OPTIONS). gix today has only the plumbing `Commit` subcommand (verify/sign/describe) — every porcelain row is currently un-wired. Closing requires reshaping `Commit(commit::Subcommands)` into `Commit(commit::Platform)` (flag-bearing top-level mirroring builtin/commit.c options[]), wiring the index→tree→commit_as flow in gitoxide_core::repository::commit, and translating cleanup/--amend/trailers/pathspec semantics. |

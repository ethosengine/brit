...
pub fn run(repo: &Path, files: Option<&str>, since: Option<&str>, pipeline: Option<&str>) -> Result<()> {
    ...
    let (changed_paths, baseline_ref, baseline_commit, head_commit) = if let Some(files) = files {
        ...
        let head_commit_sha =
            rakia_brit::changes::head_commit(&repo).map_err(|e| CliError::ChangeDetection(format!("{e}")))?;
        let baseline_commit_sha =
            rakia_brit::changes::resolve_ref(&repo, since).map_err(|e| CliError::ChangeDetection(format!("{e}")))?;
...
fn compute_fingerprints(
    ...
    let repo = gix::discover(repo_path).map_err(|e| CliError::Args(format!("repo open failed: {e}")))?;
    ...
    for level in &plan.levels {
        for (step, _reasons) in level {
            ...
            let fp = brit_graph::fingerprint::ContentFingerprint::from_repo_globs(&repo, commit_id, &all_patterns)
                .map_err(|e| CliError::Args(format!("fingerprint failed for step '{}': {e}", step.qualified_name)))?;
